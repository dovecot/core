AC_DEFUN([DOVECOT_SSL], [
  ...
  AS_IF([test $want_openssl != no && test $have_ssl = no], [
    AS_IF([test "$PKG_CONFIG" != "" && $PKG_CONFIG --exists openssl 2>/dev/null], [
      PKG_CHECK_MODULES(SSL, openssl)
      CFLAGS="$CFLAGS $SSL_CFLAGS"
      have_openssl=yes
    ], [
      dnl * openssl 0.9.8 wants -ldl and it's required if there's only .a lib
      AC_CHECK_LIB(ssl, SSL_read, [
        AC_CHECK_HEADERS(openssl/ssl.h openssl/err.h, [
          ...
          AS_IF([test $want_openssl = yes], [
            AC_MSG_ERROR(Can't build with OpenSSL: openssl/ssl.h or openssl/err.h not found)
          ])
        ])
      ], [
        AS_IF([test $want_openssl = yes], [
          AC_MSG_ERROR(Can't build with OpenSSL: libssl not found)
        ])
      ], -lcrypto $DLLIB)
    ])
    AS_IF([test "$have_openssl" = "yes"], [
      ...
      AS_IF([test $ssl_version_ge_101 = false], [

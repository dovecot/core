...
static int o_stream_zstd_send_output(struct zstd_ostream *zstream) {
	...
	ret = o_stream_send(zstream->ostream.parent,
			zstream->output.dst, zstream->output.pos);
	if(ret < 0) {
...
static size_t o_stream_zstd_get_buffer_used_size(
		const struct ostream_private *stream){
	struct zstd_ostream *zstream = (struct zstd_ostream *)stream;
	return zstream->output.pos + o_stream_get_buffer_used_size(stream->parent);	
}

static size_t o_stream_zstd_get_buffer_avail_size(
		const struct ostream_private *stream){
...
struct ostream *o_stream_create_zstd(struct ostream *output, int level)
{
	...
	zstream->ostream.get_buffer_used_size = o_stream_zstd_get_buffer_used_size;
	...
	return o_stream_create(&zstream->ostream, output, o_stream_get_fd(output));
}
#endif

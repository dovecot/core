	...
	enum mail_search_mime_arg_type type)
{
	...
	smarg = mail_search_mime_build_new(ctx, type);
	smarg->field_name = p_strdup(ctx->ctx->pool, field_name);
...
static struct mail_search_mime_arg *
mail_search_mime_lines(struct mail_search_mime_build_context *ctx)
{
	struct mail_search_mime_arg *smarg;
	enum mail_search_mime_arg_type type;
	const char *key, *value;
	...
	if (mail_search_parse_key(ctx->ctx->parser, &key) <= 0) {
		ctx->ctx->_error = "Invalid MIMEPART LINES key";
		return NULL;
	}

	key = t_str_ucase(key);
	if (strcmp(key, "FEWER") == 0)
		type = SEARCH_MIME_LINES_FEWER;
	else 	if (strcmp(key, "MORE") == 0)
		type = SEARCH_MIME_LINES_MORE;
	else {
		type = SEARCH_MIME_LINES_EQUAL;
		value = key;
	}

	if (type != SEARCH_MIME_LINES_EQUAL &&
		mail_search_parse_string(ctx->ctx->parser, &value) < 0) {
		ctx->ctx->_error = "Invalid MIMEPART LINES value";
		return NULL;
	}

	if (str_to_uint(value, &lines) < 0) {
		ctx->ctx->_error = "Invalid MIMEPART LINES count";
		return NULL;
	}

	smarg = mail_search_mime_build_new(ctx, type);
	smarg->value.number = lines;
	return smarg;
}
...
mime_register_args[] = {
	...
	/* message */
	// { "BCC", mail_search_mime_bcc },// FIXME
	// { "CC", mail_search_mime_cc },// FIXME
	// { "FROM", mail_search_mime_from },// FIXME
	// { "IN-REPLY-TO", mail_search_mime_in_reply_to },// FIXME
	// { "SENDER", mail_search_mime_sender },// FIXME
	// { "SUBJECT", mail_search_mime_subject },// FIXME
	// { "TO", mail_search_mime_to },// FIXME

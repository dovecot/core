...
// GromySkynet
/*
Добавил doveadm.h из-за ошибки сборки модуля doveadm-exec.c при импорте
doveadm-cmd-parse.h In file included from doveadm-exec.c:21:
doveadm-cmd-parse.h:113:34: error: field ‘conn_type’ has incomplete type
  113 |         enum doveadm_client_type conn_type;
*/
#include "doveadm.h"
// GromySkynet END

#define DOVEADM_CMD_PARAMS_START \
	.parameters = (const struct doveadm_cmd_param[]) \
	{
#define DOVEADM_CMD_PARAM(optP, nameP, typeP, flagP) \
	DOVEADM_CMD_PARAMKV(optP, nameP, nameP, typeP, flagP)
#define DOVEADM_CMD_PARAMKV(optP, nameP, keyP, typeP, flagP) \
	{ .short_opt = optP, \
	  .name = nameP, \
	  .key = keyP, \
	  .type = typeP, \
	  .flags = flagP },
#define DOVEADM_CMD_PARAMS_END \
	{ \
		.short_opt = '\0', .name = NULL, .type = CMD_PARAM_BOOL, \
		.flags = CMD_PARAM_FLAG_NONE \
	} \
	}
...
struct doveadm_cmd_context *doveadm_cmd_context_create(
	enum doveadm_client_type conn_type,
	bool forced_debug);
void doveadm_cmd_context_unref(struct doveadm_cmd_context **cctx);

void doveadm_cmd_context_replace_set_event(
	struct doveadm_cmd_context *cctx,
	struct event *set_event);

/* Returns 0 if success, -1 if parameters were invalid. */
int doveadm_cmdline_run(
	int argc,
	const char *const argv[],
	struct doveadm_cmd_context *cctx);

bool doveadm_cmd_param_bool(
...
bool doveadm_cmd_param_int64(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	int64_t *value_r);
bool doveadm_cmd_param_uint64(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	uint64_t *value_r);
bool doveadm_cmd_param_int32(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	int32_t *value_r);
bool doveadm_cmd_param_uint32(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	uint32_t *value_r);
bool doveadm_cmd_param_str(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	const char **value_r);
bool doveadm_cmd_param_ip(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	struct ip_addr *value_r);

bool doveadm_cmd_param_array_get(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	ARRAY_TYPE(const_string) *value_r);
bool doveadm_cmd_param_array_append(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	ARRAY_TYPE(const_string) *dest);
bool doveadm_cmd_param_array(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	const char *const **value_r);

bool doveadm_cmd_param_istream(
	const struct doveadm_cmd_context *cctx,
	const char *name,
	struct istream **value_r);

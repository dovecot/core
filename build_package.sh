...
sed -i "s/Version:.*/Version: ${VERSION}/" $DIR_FILES/Pack/src/DEBIAN/control
sed -i "s/Source:.*/Source: r7mdaserver (${VERSION})/" $DIR_FILES/Pack/src/DEBIAN/control
sed -i "s/Package:.*/Package: dovecot/" $DIR_FILES/Pack/src/DEBIAN/control

dpkg-deb -b $DIR_FILES/Pack/src $DIR_FILES/Packages/dovecot_${VERSION}.deb
